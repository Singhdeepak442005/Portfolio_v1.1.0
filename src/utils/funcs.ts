...
import { getCertificateSuggestions } from "./certificateData";
...
export const argTab = (
  ...
  // 1) handle cd autocompletion with current directory awareness
  if (inputVal === "cd") {
    setInputVal("cd ");
    return [];
  }

  if (_.startsWith(inputVal, "cd ")) {
    const partial = inputVal.substring(3).trim();
    const rootSections = [
      "welcome",
      "about",
      "education",
    ...
    if (currentDir === "certificates") {
      const certHints = getCertificateSuggestions(partial, currentDir);
      if (certHints.length === 1) {
        setInputVal(`cd ${certHints[0]}`);
        setHints([]);
        return [];
      }
      return certHints;
    ...
    const sectionHints = rootSections.filter((section) =>
      _.startsWith(section, partial.toLowerCase())
    );

    if (sectionHints.length === 1) {
      setInputVal(`cd ${sectionHints[0]}`);
      setHints([]);
      return [];
    }
